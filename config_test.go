...
Second line."""
sync_command = "receiver"
sync_args = ["sync", "a path with spaces"]
send_command = "sender"
send_args = ["{recipient}"]
`)
	...
	if !filepath.IsAbs(cfg.MaildirRoot) || !reflect.DeepEqual(cfg.SyncArgs, commandArgs{"sync", "a path with spaces"}) {
		t.Fatalf("paths/args: %+v", cfg)
	}
	cfg, err = loadConfig([]string{"-model=cli-model", "-config", p, "-reply-anyone=false", "-sync-arg=new", "-sync-arg=", "-send-arg=-t"}, io.Discard)
	if err != nil {
		t.Fatal(err)
	}
	if cfg.Model != "cli-model" || cfg.ReplyAnyone || !reflect.DeepEqual(cfg.SyncArgs, commandArgs{"new", ""}) || !reflect.DeepEqual(cfg.SendArgs, commandArgs{"-t"}) {
		t.Fatalf("overrides: %+v", cfg)
	}
}

func TestTOMLConfigErrors(t *testing.T) {
	for _, body := range []string{`modle = "typo"`, `interval = "later"`, `sync_args = "sync"`, `reply_anyone = "yes"`, `model = "a"
model = "b"`, `model = [`} {
		p := configFile(t, body)
